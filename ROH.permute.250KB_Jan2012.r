...
##############################################################################
# NOTE: MAKE SURE INTEGER VALUE IN read.table FUNCTION THAT READS IN DATA.MATRIX IS SET TO NUMBER OF COLUMNS IN DATA THAT ARE BINS
...
ROH.FINAL <- read.table(data.matrix.name,header=TRUE,fill=TRUE,colClasses=c(rep('character',4),rep('integer',11472)))
