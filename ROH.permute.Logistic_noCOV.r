...
ROH.FINAL <- read.table(data.matrix.name,header=TRUE,fill=TRUE,colClasses=c(rep('character',6),rep('numeric',20),rep('integer',11427)))
...
logistic.model <- glm(as.factor(ROH.LOGISTIC$PHE) ~ ROH.LOGISTIC[,j], family=binomial("logit"),data=ROH.LOGISTIC)
...
# 
if (z==1){
write.table(logistic.matrix,file="ROH.map.noCOV.data",quote=FALSE,row.names=TRUE,col.names=FALSE)# write out un-permuted results
}else{
write.table(logistic.matrix,file=paste("temp.noCOV.ROH",z-1,"",sep=""),quote=FALSE,row.names=TRUE,col.names=FALSE)# write each iteration to a temporary file
}
...
temp.roh <- matrix(scan(paste("temp.noCOV.ROH",k,"",sep=""),what='character'),nrow=8,ncol=(ncol(ROH.FINAL)-25),byrow=TRUE)
temp.roh <- as.matrix(temp.roh)             
roh.append <- rbind(roh.append,temp.roh)
#system(paste("rm temp.noCOV.ROH",k,"",sep="")) # remove temp file after reading in # UNCOMMENT AFTER TESTING 
}# end loop k

roh.append <-roh.append[2:nrow(roh.append),] # remove empty top row of merged .fam files
write.table(roh.append,file="ROH.map.noCOV.permute",quote=FALSE,row.names=FALSE,col.names=FALSE,append=FALSE)#
#
...
cores <- 500 #Number of cores to use
