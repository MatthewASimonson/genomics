...
mds.data <- as.data.frame(mds.append[,c(2,4:23)]) # INCLUDES 20 PCA'S
...
map.matrix.key[2,i] <- max(map.matrix.key.fill[,i],na.rm=TRUE) # Based on last run position across all runs in each bin
print(i)
}
...
fill.matrix <- matrix(0,nrow=nrow(no_runs.subjects),ncol=2875)
