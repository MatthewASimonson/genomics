setwd("/STATGEN/home/whme1016/dbgap/MESA/Cleaning")

dataset.bfile <- 'MESA_PLINK_consent1_update'
...
# completed this step, August 29, 2012 11:30AM
...
# completed this step, August 29 2012, 11:58 AM
...
# completed this step August 29 2012, 12:35 PM
...
# this step completed August 31 2012, 9:32 AM
...
# finished this step August 31 2012, 11:34 AM
...
system("nohup /home/whme1016/dbgap/MESA/Cleaning/shellfish/shellfish.py --pca --numpcs 20 --maxprocs 5 --file shellfish_MDS --out 7_shell_mds.correct &") ##

# this step finished Sept. 5 2012 at 11:46 AM
...
}' 7_shell_mds.correct.evecs  > 7_shell_mds.tevecs"})
...
# Plot MDS data:
IND.dat <- read.table("flip.gwas.fam",header=FALSE) # read and merge IID's

sample.index <- 270:nrow(MDS)
JPT.index <- 91:180
YRI.index <- 181:270
CEU.index <- 61:90
...
legend(0.025,0.05,c('Sample (European Ancestry)','HapMap YRI','HapMap JPT+CHB'),text.col=c('blue','orange','purple'))
#
center.cau.x <- median(c(MDS$C1[sample.index][which(MDS$C1[sample.index]<0)]))
center.cau.y <- median(c(MDS$C2[sample.index][which(MDS$C2[sample.index]<0)]))
...
# make sure "keepstrat.list.txt" written into working directory
# system('ls')

# this step finished Sept. 06 2012 4:13 pm
...
# SOMETHING IS GOING WRONG HERE WERE THE RS SNP NAMES ARE NOT KEPT;
# I THINK WE WANT TO USE THE 7_step_shellfish BFILE INSTEAD OF THE 5.STEP.UNRELATED FILE???????
# Or maybe the flip.gwas file???
# But which one???
...
##############################
# Step 11: recode affy to rs #
##############################
# Old step 11 was not neccesary

bim <- read.table("10.step.HWE.bim",header=FALSE)
key <- read.csv("GenomeWideSNP_6.na27.annot.csv",header=TRUE)

snp.index <- match(bim$V2,key$Probe.Set.ID)
rs.key <- cbind.data.frame(key$Probe.Set.ID,key$dbSNP.RS.ID)
write.table(rs.key,file="rs.key",quote=FALSE,row.names=FALSE,col.names=FALSE)

system("plink --bfile 10.step.HWE --update-map rs.key --update-name --make-bed --out 11.step.clean")
...
sample <- read.table("shellfish_MDS.sample",header=TRUE)
names(sample) <- c('IID','IID2')

MDS.samp <- cbind.data.frame(sample[2:nrow(sample),],MDS)

samp.used <-  read.table("11.step.hap.fam",header=FALSE)
names(samp.used) <- c('FID','IID')
MDS.used <- merge(MDS.samp,samp.used,by='IID')

batch.covar <- read.table("MESA.batch.covar",header=FALSE)
names(batch.covar) <- c('IID','SEX','AGE','BATCH')
total.covars <- merge(batch.covar,MDS.used,by='IID')

covar.file <- cbind.data.frame(total.covars$FID,total.covars$IID,total.covars$SEX,total.covars$AGE,total.covars$BATCH,total.covars[,9:28])
names(covar.file) <-  c('FID','IID','SEX','AGE','BATCH','C1','C2','C3','C4','C5','C6','C7','C8','C9','C10','C11','C12','C13','C14','C15','C16','C17','C18','C19','C20')
...
# FINISHED COMPLETELY CLEANING MESA SEPT 7 2012 AT 11:13 AM
