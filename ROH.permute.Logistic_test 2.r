...
linear.matrix <- rbind(Beta0,Beta1,Beta0_P_val,Beta1_P_val,Beta0_StandardErr,Beta1_StandardErr,Tot_resid_deviance,ML_iterations)
...
linear.matrix[7,j-4] <- linear.model$deviance # Tot_resid_deviance
linear.matrix[8,j-4] <- linear.model$iter # number of iterations until convergence of maximum liklihood (25 means no-convergence)
} # end loop j
