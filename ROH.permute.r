...
ML_iterations <- rep(0,(ncol(ROH.FINAL)-24))
...
for( j in 25:(ncol(ROH.LOGISTIC))){# loop through each megabase | (ncol(ROH.LOGISTIC)) CHANGE
logistic.model <- glm(PHE ~ ROH.LOGISTIC[,j] + SEX + DATA + as.numeric(C1) + as.numeric(C2) + as.numeric(C3) + as.numeric(C4) + as.numeric(C5) + as.numeric(C6) + as.numeric(C7) + as.numeric(C8) + as.numeric(C9) + as.numeric(C10), family=binomial("logit"),data=ROH.LOGISTIC)
...
logistic.matrix[8,j-24] <- logistic.model$iter # number of iterations until convergence of maximum liklihood (25 means no-convergence)
} # end loop j
